...
contract MorpheusBuyAndBurn is ReentrancyGuard, Ownable2Step {
    ...
    event BuyAndBurn(uint256 indexed titanXAmount, uint256 indexed morpheusBurnt, address indexed caller);
     ...
     */
    function swapTitanXForMorpheusAndBurn(uint256 _amountDragonxMin, uint32 _deadline)
        external
        nonReentrant
        intervalUpdate
    {
        ...
        uint256 dragonxAmount = _swapTitanForDragonx(titanXToSwapAndBurn, _amountDragonxMin, _deadline);
        uint256 morpheusAmount = _swapDragonxForMorpheus(dragonxAmount, _deadline);
     ...
     */
    function addLiquidityToMorpheusDragonxPool(uint32 _deadline, uint256 _amountDragonxMin) external onlyOwner {
        ...
        uint256 dragonxReceived = _swapTitanForDragonx(INITIAL_TITAN_X_FOR_LIQ, _amountDragonxMin, _deadline);
    ...
    function setSlippageForDragonxToMorpheus(uint8 _newSlippage) external onlyOwner {
    ...
    function distributeTitanXForBurning(uint256 _amount) external {
        ...
        if (msg.sender != address(morpheusToken.minting())) revert OnlyMinting();
    ...
    function burnFees() external returns (uint256 amount0, uint256 amount1) {
        ...
        (uint256 dragonxAmount,) = _lp.isDragonxToken0 ? (amount0, amount1) : (amount1, amount0);
     ...
     */
    function getMorpheusQuoteForDragonx(uint256 baseAmount) public view returns (uint256 quote) {
        ...
        quote = OracleLibrary.getQuoteForSqrtRatioX96(sqrtPriceX96, baseAmount, address(dragonx), address(morpheusToken));
    }
     ...
     */
    function getDragonxQuoteForTitanX(uint256 titanXAmount) external view returns (uint256 quote) {
     ...
     */
    function _swapTitanForDragonx(uint256 amountTitan, uint256 amountDragonxMin, uint256 _deadline)
        private
        returns (uint256 _dragonxAmount)
    {
        ...
        uint256[] memory returnedOutputAmounts = IUniswapV2Router02(UNISWAP_V2_ROUTER).swapExactTokensForTokens(
            amountTitan, amountDragonxMin, path, address(this), _deadline
        );

        _dragonxAmount = returnedOutputAmounts[returnedOutputAmounts.length - 1];
    }
     ...
     */
    function _swapDragonxForMorpheus(uint256 amountDragonx, uint256 _deadline) private returns (uint256 _titanAmount) {
        // wake-disable-next-line
        dragonx.approve(UNISWAP_V3_ROUTER, amountDragonx);
        // Setup the swap-path, swapp
        bytes memory path = abi.encodePacked(address(dragonx), POOL_FEE, address(morpheusToken));

        uint256 expectedMorpheusAmount = getMorpheusQuoteForDragonx(amountDragonx);

        // Adjust for slippage (applied uniformly across both hops)
        uint256 adjustedMorpheusAmount = (expectedMorpheusAmount * (100 - dragonxToMorpheusSlippage)) / 100;
     ...
     */
    function _sortAmounts(uint256 dragonxAmount, uint256 morpheusAmount)
        internal
        view
        returns (
            uint256 amount0,
        ...
        )
    {
        ...
        (token0, token1) = _dragonx < _morpheus ? (_dragonx, _morpheus) : (_morpheus, _dragonx);
        (amount0, amount1) = token0 == _dragonx ? (dragonxAmount, morpheusAmount) : (morpheusAmount, dragonxAmount);
